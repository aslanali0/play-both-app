  ...
  return (
           ...
           <li className="group flex items-center justify-between p-5 rounded-2xl bg-white/5 hover:bg-neon-cyan/20 border border-transparent hover:border-neon-cyan/40 transition-all active:scale-95 w-full"> 
              <div className="flex col">
                <div><h2>{songData?.title}</h2>
        
                <span className="text-gray-400 text-[10px] uppercase tracking-tighter font-mono italic">{songData?.album_title}</span>
                </div>
                <button
              ...
              <a target='_blank' href={`https://www.youtube.com/watch?v=${songData?.youtube_url}`}>
                <img className='w-70' src={`https://img.youtube.com/vi/${songData?.youtube_url}/hqdefault.jpg`} />
              </a>
            </li>
  )
}
