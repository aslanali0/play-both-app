...
import type { Game, Song, Favorite } from "../types/content.ts";
...
const GameCard = ({ gameData }: { gameData: Game | null }) => {
  ...
  return (
    <div className="text-white relative animate-slide-up transition-all group w-full max-w-6xl mx-auto mt-10">
      {!gameData && (
        <button disabled>
      ...
      <div key={gameData?.steam_id} className="w-full">
        {gameData ? (
          <div className="home-element p-10 w-full animate-slide-up mx-auto mt-12 transition-all ">
            <h1 className="text-6xl font-black uppercase mb-10">
              {gameData.title}
            </h1>
            <img
              className="w-75 card m-4 rounded"
              src={gameData.image_url || ""}
            />
            <ul key={pageCounter} className="w-full flex flex-col gap-6 p-4">
              {processedData
                ?.slice(pageCounter * 10, (pageCounter + 1) * 10)
                .map((item) =>
                  item ? (
            ...
            {processedData && (
              <div className="text-center">
                {processedData?.slice((pageCounter - 1) * 10, pageCounter * 10)
                  .length > 0 && (
                  <button
                    className="underline text-blue-400 cursor-pointer mr-10"
                    onClick={() => handleNextBackPage(-1)}
                  >
                    {" "}
                    &lt; Previous Page{" "}
                  </button>
